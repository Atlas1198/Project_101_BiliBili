...
void EffectManager::Initialize(TextureManager& textureManager, MeshManager& meshManager)
{
	...
	AddEffectCommand({ EFFECT_TYPE::PARTICLE_POINT, XMFLOAT3(0.0f, 0.0f, 10.0f), XMFLOAT2(1.0f, 1.0f) });
}
...
)
{
	...
	{//アクティブかつ描画フラグが立っている場合
		std::vector<EffectRenderInfo> submitInfos;	//Rendererへの提出用描画情報構造体配列
		...
		{
			renderer.SubmitToEffectList(info);
		}
	}
}
...
void EffectManager::PushEffectFromQueue()
{
	for (auto& command : m_pEffectQueue)
	{
		...
		if (freeEffect)
		{
			...
			freeEffect->SetSize(command.size);
