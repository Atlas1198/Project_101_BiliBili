...
)
{
	...
	//プレイヤーアイコンUIオブジェクト配列生成
	{
		...
		float basePositionY = m_pCharacterIconBase->GetLocalTransform().position.y;
		float baseScaleY = m_pCharacterIconBase->GetLocalTransform().scale.y;
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			Transform3D transform = m_pPlayerIcons[i]->GetLocalTransform();
			transform.position.x = positionX;
			m_pPlayerIcons[i]->SetLocalTransform(transform);
		}

		//プレイヤーアイコンのXスケールオフセット計算
		for(int i = 0; i < 4; ++i)
		{
			float centerX = m_pPlayerBackgroundsNormal[0]->GetLocalTransform().position.x;
			float sub = m_pPlayerIcons[i]->GetLocalTransform().position.x - centerX;
...
void CharacterUIManager::UpdateOverride()
{
	...
	//プレイヤーアイコンのサイズ変更
	{
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			{
				m_pPlayerIcons[i]->SetLocalTransform(
					Transform3D{
						m_pPlayerIcons[i]->GetLocalTransform().position,
						{ 88.0f * scaleFactor, 107.0f * scaleFactor, 1.0f },
						m_pPlayerIcons[i]->GetLocalTransform().rotation
					}
				);
			}
		}
	...
	//次のシーンへ進むアイコンの座標移動
	{
		Transform3D transform = m_pGoToNextSceneIcon->GetLocalTransform();
		const float moveSpeedCoefficient = 100.0f - transform.position.x;	//移動速度係数計算
		const float moveSpeed = moveSpeedCoefficient * 0.2f;	//移動速度計算

		if(m_isGoToNextSceneIconVisible)
		{//表示中はX座標を中心に移動
			transform.position.x = (std::min)(transform.position.x + moveSpeed, 0.0f);
			m_pGoToNextSceneIcon->SetLocalTransform(transform);
		}
		else
		{//非表示中は画面外へ移動
			transform.position.x = (std::max)(transform.position.x - moveSpeed, -m_screenWidth * 0.5f - 1917.0f * 0.5f);
			m_pGoToNextSceneIcon->SetLocalTransform(transform);
		}
	}
...
{
	//移動量計算
	const float backGroundIconWidth = m_pPlayerBackgroundsNormal[0]->GetLocalTransform().scale.x;								//背景アイコンの幅
	const float basePositionX = -m_pCharacterIconBase->GetLocalTransform().scale.x * 0.5f + m_pPlayerBackgroundsSelected[0]->GetLocalTransform().scale.x * 0.5f;	//ベース位置X
	const float lengthBetweenIcons = (m_pCharacterIconBase->GetLocalTransform().scale.x - backGroundIconWidth * 4.0f) / 3.0f;	//アイコン間の長さ
	const float moveAmount = backGroundIconWidth + lengthBetweenIcons;															//移動量

	//移動処理
	Transform3D transform = m_pPlayerIcons[playerIndex]->GetLocalTransform();								//現在の変換情報取得
	float newPositionX = basePositionX + characterIndex * moveAmount + playerIconScaleOffsets[playerIndex];	//新しいX位置計算
	transform.position.x = newPositionX;																	//X位置更新

	//変換情報設定
	m_pPlayerIcons[playerIndex]->SetLocalTransform(transform);
}
...
void CharacterUIManager::ActivateSelectedPlayerBackground(int playerIndex, int characterIndex)
{
	...
	if (playerIndex >= 0 && playerIndex < 4)
	{
		...
		Transform3D transformBG = m_pPlayerBackgroundsSelected[playerIndex]->GetLocalTransform();
		transformBG.position = m_pPlayerBackgroundsNormal[characterIndex]->GetLocalTransform().position;
		m_pPlayerBackgroundsSelected[playerIndex]->SetLocalTransform(transformBG);

		Transform3D transformIcon = m_pPlayerIcons[playerIndex]->GetLocalTransform();
		const float iconScaleFactor = 1.2f;
		transformIcon.scale = { 88.0f * iconScaleFactor, 107.0f * iconScaleFactor, 1.0f };
		m_pPlayerIcons[playerIndex]->SetLocalTransform(transformIcon);
