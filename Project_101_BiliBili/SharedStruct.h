...
struct UVRect
{
	float u = 0.0f;		//UV矩形の左上X
	float v = 0.0f;		//UV矩形の左上Y
...
struct alignas(256) TransformCB
{
	...
	DirectX::XMFLOAT4 objectColor;	//オブジェクトカラーRGBA
...
struct TexSplitInfo
{
	int index = 0;			//現在のスプライトのインデックス
	int cols = 1;			//スプライトシートの列数
	int rows = 1;			//スプライトシートの行数
	int total = 1;			//スプライトの総数(= cols * rows)
	int frameCount = 0;		//経過フレーム数
	int updateRate = 0;		//更新頻度(経過フレーム数がこの値に達したら更新)
};
